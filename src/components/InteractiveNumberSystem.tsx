...
export default function InteractiveNumberSystem() {
  ...
  const tabs = [
    { id: 'decimal', label: 'Desimal (10)', icon: Hash, color: 'text-blue-500', bg: 'bg-blue-500' },
    { id: 'binary', label: 'Biner (2)', icon: Zap, color: 'text-emerald-500', bg: 'bg-emerald-500' },
    { id: 'octal', label: 'Oktal (8)', icon: Settings2, color: 'text-yellow-500', bg: 'bg-yellow-500' },
    { id: 'hex', label: 'Heksadesimal (16)', icon: Grip, color: 'text-purple-500', bg: 'bg-purple-500' },
  ];
          ...
          {activeTab === 'decimal' && (
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground">Sistem yang kita gunakan sehari-hari. Memiliki 10 simbol (0-9).</p>
              </div>
                ...
                {decVals.map((val, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center gap-4">
                      <div className="text-sm font-semibold text-muted-foreground">10<sup className="text-[10px]">{power}</sup> = {multiplier}</div>
                      <div className="flex flex-col items-center gap-2 bg-secondary/30 p-3 md:p-4 rounded-xl border border-border/50">
                        <button onClick={() => handleDecChange(idx, 1)} className="w-10 h-8 md:h-10 rounded-lg hover:bg-secondary flex items-center justify-center text-xl font-bold">▲</button>
                        <div className="text-5xl md:text-6xl font-black text-blue-500 w-16 text-center">{val}</div>
                        <button onClick={() => handleDecChange(idx, -1)} className="w-10 h-8 md:h-10 rounded-lg hover:bg-secondary flex items-center justify-center text-xl font-bold">▼</button>
                      </div>
                      <div className="text-lg md:text-xl font-bold text-blue-500/70">{val} × {multiplier}</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-6 md:mt-8 p-4 md:p-6 bg-blue-500/5 rounded-2xl border border-blue-500/20 text-center">
                ...
                <div className="text-4xl font-black text-foreground">
                  = <span className="text-blue-500">{decVals[0] * 100 + decVals[1] * 10 + decVals[2]}</span>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {activeTab === 'binary' && (
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground">Bahasa mesin. Hanya memiliki 2 simbol: 0 (Mati) dan 1 (Menyala).</p>
              </div>
                ...
                {binVals.map((val, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center gap-2 md:gap-3">
                      <div className="text-[10px] md:text-xs font-semibold text-muted-foreground hidden sm:block">2<sup className="text-[8px] md:text-[10px]">{power}</sup></div>
                      <div className="text-xs md:text-sm font-bold text-foreground">{multiplier}</div>
                      ...
                      <button 
                        ...
                        className={`w-9 h-14 sm:w-12 sm:h-20 md:w-16 md:h-24 rounded-lg md:rounded-xl border-2 flex flex-col items-center justify-between py-1 md:py-2 transition-all duration-300 relative overflow-hidden group shrink-0 ${
                          isOn ? 'border-emerald-500 bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'border-border/50 bg-secondary/30 hover:border-emerald-500/50'
                        }`}
                      >
                        <div className={`text-sm sm:text-lg md:text-2xl font-black z-10 transition-colors ${isOn ? 'text-emerald-500 drop-shadow-[0_0_5px_rgba(16,185,129,0.8)]' : 'text-muted-foreground'}`}>
                          {val}
                        </div>
                        <div className={`w-4 h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center z-10 transition-colors ${isOn ? 'bg-emerald-500 text-white shadow-[0_0_10px_rgba(16,185,129,0.8)]' : 'bg-secondary text-muted-foreground'}`}>
                      ...
                      <div className={`text-xs md:text-sm font-bold transition-colors ${isOn ? 'text-emerald-500' : 'text-muted-foreground/30'}`}>
                        {isOn ? multiplier : 0}
                      </div>
                    </div>
                  );
              ...
              <div className="mt-6 md:mt-8 p-4 md:p-6 bg-emerald-500/5 rounded-2xl border border-emerald-500/20 text-center">
                <div className="text-xs md:text-sm text-muted-foreground font-mono mb-2 md:mb-4 flex flex-wrap justify-center gap-2 max-w-2xl mx-auto leading-relaxed">
                  {binVals.map((val, idx) => {
                    const multiplier = Math.pow(2, 7 - idx);
                    const isLast = idx === 7;
                    return (
                      <span key={idx} className={val === 1 ? 'text-emerald-500 font-bold' : ''}>
                ...
                <div className="text-4xl font-black text-foreground flex items-center justify-center gap-4">
                  ...
                  <span className="text-xl text-muted-foreground mt-2">Desimal</span>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {activeTab === 'octal' && (
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground">Sering digunakan dalam komputasi awal. Memiliki 8 simbol (0-7).</p>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-8 md:gap-12">
                {octVals.map((val, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center gap-4">
                      <div className="text-sm font-semibold text-muted-foreground">8<sup className="text-[10px]">{power}</sup> = {multiplier}</div>
                      <div className="flex flex-col items-center gap-2 bg-secondary/30 p-3 md:p-4 rounded-xl border border-border/50">
                        <button onClick={() => handleOctChange(idx, 1)} className="w-10 h-8 md:h-10 rounded-lg hover:bg-secondary flex items-center justify-center text-xl font-bold">▲</button>
                        <div className="text-5xl md:text-6xl font-black text-yellow-500 w-16 text-center drop-shadow-sm">{val}</div>
                        <button onClick={() => handleOctChange(idx, -1)} className="w-10 h-8 md:h-10 rounded-lg hover:bg-secondary flex items-center justify-center text-xl font-bold">▼</button>
                      </div>
                      <div className="text-lg md:text-xl font-bold text-yellow-500/70">{val} × {multiplier}</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-6 md:mt-8 p-4 md:p-6 bg-yellow-500/5 rounded-2xl border border-yellow-500/20 text-center">
                ...
                <div className="text-4xl font-black text-foreground">
                  = <span className="text-yellow-600 dark:text-yellow-400">{octVals[0] * 64 + octVals[1] * 8 + octVals[2]}</span>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {activeTab === 'hex' && (
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground">Standar untuk alamat memori dan warna. Memiliki 16 simbol (0-9, A-F).</p>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-8 md:gap-16">
                {hexVals.map((val, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center gap-4">
                      <div className="text-sm font-semibold text-muted-foreground">16<sup className="text-[10px]">{power}</sup> = {multiplier}</div>
                      
                      <div className="flex flex-col items-center gap-2 relative">
                        <button onClick={() => handleHexChange(idx, 1)} className="w-12 h-8 md:h-10 rounded-lg hover:bg-secondary flex items-center justify-center text-xl font-bold">▲</button>
                        ...
                        <div className={`w-24 h-28 md:w-28 md:h-32 flex flex-col items-center justify-center rounded-2xl border-4 shadow-lg ${isAlpha ? 'bg-purple-500/10 border-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.3)]' : 'bg-secondary/30 border-border/50'}`}>
                          <div className={`text-6xl md:text-7xl font-black ${isAlpha ? 'text-purple-500' : 'text-foreground'}`}>
                            {char}
                          </div>
                        </div>
                        ...
                        <AnimatePresence>
                          {isAlpha && (
                            <motion.div 
                              ...
                              exit={{ opacity: 0, y: 10 }}
                              className="absolute top-1/2 -right-14 md:-right-16 -translate-y-1/2 bg-purple-500 text-white text-xs md:text-sm font-bold px-2 md:px-3 py-1 md:py-1.5 rounded-lg shadow-xl z-20"
                            >
                              = {val}
                              <div className="absolute top-1/2 -left-1 -translate-y-1/2 border-y-4 border-y-transparent border-r-4 border-r-purple-500"></div>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>

                      <div className={`text-lg md:text-xl font-bold ${isAlpha ? 'text-purple-500' : 'text-foreground/70'}`}>
                        {val} × {multiplier}
                      </div>
                    </div>
                  );
              ...
              <div className="mt-6 md:mt-8 p-4 md:p-6 bg-purple-500/5 rounded-2xl border border-purple-500/20 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="text-5xl font-black text-foreground">
                    = <span className="text-purple-500 drop-shadow-md">{hexVals[0] * 16 + hexVals[1]}</span>
                  </div>
                </div>
              </div>
            </motion.div>
